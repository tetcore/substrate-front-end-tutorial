...
describe('createClass', () => {
  ...
  it('should memoize from strings', () => {
    const a = (0, _createType.createClass)('BabeWeight');
    const b = (0, _createType.createClass)('BabeWeight');
    expect(a).toBe(b);
  });
  it('should memoize from Text', () => {
    const a = (0, _createType.createClass)(new _Text.default('TrieId'));
    const b = (0, _createType.createClass)(new _Text.default('TrieId'));
    expect(a).toBe(b);
  });
  it('should memoize from string/Text combo', () => {
    const a = (0, _createType.createClass)(new _Text.default('AuthorityWeight'));
    const b = (0, _createType.createClass)('AuthorityWeight');
    expect(a).toBe(b);
  });
});
describe('createType', () => {
  ...
  it('allows creation of a [u16; 4]', () => {
    expect((0, _createType.default)('[u16; 4]', [0x1200, 0x2300, 0x4500, 0x6700]).toU8a()).toEqual(new Uint8Array([0x00, 0x12, 0x00, 0x23, 0x00, 0x45, 0x00, 0x67]));
  });
  it('throw error when create base is a StorageData with null value and isPedantic is true', () => {
    ...
    expect(() => (0, _createType.default)('DoubleMap<Vec<(BlockNumber,EventIndex)>>', base, true)).toThrow(/ Input doesn't match output, received 0x, created 0x00/);
  });
  it('throw error when create base is a StorageData with null value and isPedantic is true', () => {
    const base = (0, _createType.default)('StorageData', null);
    expect(() => (0, _createType.default)('Vec<(BlockNumber,EventIndex)>', base, true)).toThrow(/Input doesn't match output, received 0x, created 0x00/);
  });
  describe('instanceof', () => {
    beforeAll(() => {
      (0, _srml.injectDefinitions)();
    ...
    it('instanceof should work (primitive type)', () => {
      const value = (0, _createType.default)('Balance', 1234);
      expect(value instanceof _Balance.default).toBe(true);
    });
    it('instanceof should work (srml type)', () => {
      const value = (0, _createType.default)('Gas', 1234);
      const Gas = (0, _createType.ClassOf)('Gas');
    ...
    it('instanceof should work (complex type)', () => {
      ...
      const value = (0, _createType.default)(complexType, {
        balance: 123,
        accountId: '',
        log: [456, 789],
        fromSrml: 0
      });
      const ComplexType = (0, _createType.createClass)(complexType);
